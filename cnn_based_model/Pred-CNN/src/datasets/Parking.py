...
def load_data(T=24, nb_flow=1, len_closeness=None, len_period=None, len_trend=None,
    ...
    data_all = []
    timestamps_all = list()
    for year in range(0, 1): # 10 Reduction dataset
    ...
    for data, timestamps in zip(data_all_mmn, timestamps_all):
        ...
        _XCPT = _XC
        if (len_period > 0) and (len_closeness> 0):
            _XCPT = np.concatenate((_XP, _XC),axis=1) # modificato per avere XPC
        if (len_period > 0):
            _XCPT = _XP # modificato per avere XPC
        if (len_trend > 0):
