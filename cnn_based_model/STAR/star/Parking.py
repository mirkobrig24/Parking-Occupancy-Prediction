...
def load_data(T=3, nb_flow=1, len_closeness=1, len_period=0, len_trend=0,
              len_test=None, len_val=None, meta_data=True, meteorol_data=False, holiday_data=False, datapath=None):
    ...
    for data, timestamps in zip(data_all_mmn, timestamps_all):
        ...
        _XCPT = _XC
        if _XP.shape[0] != 0 and _XCPT.shape[0] !=0:
            _XCPT = np.concatenate((_XCPT, _XP),axis=1)
        if  _XCPT.shape[0] ==0 and _XP.shape[0] != 0:
           _XCPT = _XP 
        if _XT.shape[0] != 0:
    ...
    if meta_data:
        ...
        metadata_dim = None
    #print('---META---', X_train_all[1].shape)
