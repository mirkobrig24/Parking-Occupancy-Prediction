...
def CLoST3D(city, X_train, y_train,
    ...
    start = Input(shape= (X_train.shape[1], X_train.shape[2] , X_train.shape[3], nb_flow))
...
def build_model(
    ...
    add_external_info, lr=0.0001, save_model_pic=None, nb_flow=1):

    model = CLoST3D(city, X_train, y_train, conv_filt, kernel_sz, mask, lstm, lstm_number, add_external_info, nb_flow)
    adam = Adam(lr=lr)
