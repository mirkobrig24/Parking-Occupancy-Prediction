...
class STMatrix(object):
    ...
    def __init__(self, data, timestamps, T=48, CheckComplete=False, Hours0_23=False):
        ...
        self.T = T
        func = my_s2t if Hours0_23 else string2timestamp
    ...
    def create_dataset_3D(self, len_closeness=3, len_trend=3, TrendInterval=7, len_period=3, PeriodInterval=1):
        ...
        while i < len(self.pd_timestamps):
            ...
            # closeness
            c_1_depends = list(depends[0])
            if (len(c_1_depends) > 0):
            ...
            #x_c_2_new = x_c_2_all[np.newaxis, :]  # (1, 3, 32, 32)
                ...
                x_c = x_c_1_new #np.vstack([x_c_1_new, x_c_2_new])  # (2, 3, 32, 32)
            ...
            if (len(p_depends) > 0):
                ...
                x_p = x_p_1_new # np.vstack([x_p_1_new, x_p_2_new])  # (2, 3, 32, 32)
            ...
            if (len(t_depends) > 0):
                ...
                x_t = x_t_1_new # np.vstack([x_t_1_new, x_t_2_new])  # (2, 3, 32, 32)
...
def timestamp2vec(timestamps):
    ...
    vec = [time.strptime(str(t[:10], encoding='utf-8'), '%Y-%m-%d').tm_wday for t in timestamps]  # python3
    #vec = [time.strptime(str(t[:10], encoding='utf-8'), '%Y-%m-%d').tm_wday for t in timestamps]  # python2
    ret = []
    for i in vec:
...
def load_data_Enea(T=24, nb_flow=2, len_closeness=None, len_period=None, len_trend=None,
    ...
    for year in range(0, 1):
        ...
        data, timestamps = load_stdata(datapath)
        # print(timestamps)
        # remove a certain day which does not have 48 timestamps
...
X_train, Y_train, X_test, Y_test, mmn, external_dim, timestamp_train, timestamp_test = \
        load_data_Enea(T=T, nb_flow=nb_flow, len_closeness=len_closeness, len_period=len_period,
                  len_trend=len_trend, len_test=len_test, meta_data=True, meteorol_data=False, holiday_data=False, datapath=DATAPATH)

CACHEDATA=True
path_cache = os.path.join('CACHE', task) # 'ST3DNet', task)
if CACHEDATA and os.path.isdir(path_cache) is False:
    os.makedirs(path_cache)
filename = os.path.join(path_cache, 'Parking_c%d_p%d_t%d'%(len_closeness, len_period, len_trend))
