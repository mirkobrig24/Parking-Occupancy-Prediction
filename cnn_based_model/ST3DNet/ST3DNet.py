...
class Recalibration(Layer):
    ...
    def build(self, input_shape):
        ...
        '''
        initial_weight_value = np.random.random((input_shape[1], 1, input_shape[2], input_shape[3])) # (c,1,h,w)
        self.W = K.variable(initial_weight_value)
        self._trainable_weights = [self.W]
    ...
    def call(self, x):
        ...
        x: (batch, c, h,w)
        '''
        double_x = tf.stack([x], axis=2) # [(batch,c,h,w)] => (batch,c,1,h,w)
        return tf.reduce_sum(double_x*self.W, 1) # (batch,1,h,w)

    def compute_output_shape(self, input_shape):
        return (input_shape[0], 1,input_shape[2],input_shape[3]) # (batch_size,1,h,w)
...
class Recalibration_T(Layer):
    ...
    def build(self, input_shape):
        ...
        input_shape: (batch, c, h, w)
        '''
        initial_weight_value = np.random.random(input_shape[1]) # [c,]:because output 1 channel
        self.W = K.variable(initial_weight_value)
        self._trainable_weights = [self.W]
    ...
    def call(self, x):
        ...
        W = tf.reshape(tf.tile(self.W, [map_height*map_width]),(nb_channel, 1, map_height, map_width)) # tile:sharing channel-wsie weight on different positions in the weekly-period recalibration block
        double_x = tf.stack([x], axis=2) # stack [(batch, c,h, w)] = (batch, c, 1, h,w)
        return tf.reduce_sum(double_x*W, 1) # (batch, 1, h, w)
...
def ST3DNet(c_conf=(6, 2, 16, 8), t_conf=(4, 2, 16, 8), external_dim=8, nb_residual_unit=4):
    ...
    main_inputs = []
    outputs = []
    if c_conf is not None: #len_closeness > 0:
    ...
    if external_dim != None and external_dim > 0:
        ...
        main_output = tf.keras.layers.Add()([main_output, external_output])
    else:
