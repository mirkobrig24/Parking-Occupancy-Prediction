...
filename = os.path.join('CACHE', task, 'Parking_c%d_p%d_t%d'%(len_closeness, len_period, len_trend))
f = open(filename, 'rb')
...
def train_model(lr, batch_size, residual_units, save_results=False, i=''):
    ...
    if (save_results):
        ...
        # save to csv
        csv_name = os.path.join('results', f'st3dnet_Parking_{task}_results.csv')
