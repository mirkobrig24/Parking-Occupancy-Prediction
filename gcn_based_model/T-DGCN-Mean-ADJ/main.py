...
def get_model(hidden, dm):
    model = None
    if args.model_name == "TDGCN":
        model = models.TDGCN(adj=dm.adj, hidden_dim=hidden)
    return model
...
def main_supervised_opt(batch_s, seq_l, hidden_d, lr):
...
def main_supervised(batch_s, seq_l, hidden_d, lr, i):
    batch_s = 2 ** int(batch_s)
    seq_l = int(seq_l)
    hidden_d = 2 ** int(hidden_d)
    lr = round(lr,5)
...
def main():
    ...
    if os.path.isdir('results') is False:
        os.mkdir('results')
    # valuto anche il learning rate    
    #with open('results/params_1H_new_iteration.json', 'w') as f:
    ...
    #with open('results/opt_1H_new_iteration.json', 'w') as f:
    #    json.dump(opt, f, indent=2)
    with open('results/opt_1H_new_iteration.json', 'r') as f:
        opt = json.load(f)

    fieldnames = ['iteration', 'val_loss', 'RMSE', 'MAE', 'accuracy', 'R2', 'ExplainedVar']

    if not os.path.isfile('results/res_new_1H.csv'):
        with open('results/res_new_1H.csv', 'w', encoding="utf-8") as file:
    ...
    for i in range(4):
        ...
        with open('results/res_new_1H.csv', 'a', encoding="utf-8", newline='') as file:
            writer = csv.writer(file)
